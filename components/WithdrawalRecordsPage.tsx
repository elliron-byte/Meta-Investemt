...
const WithdrawalRecordsPage: React.FC<WithdrawalRecordsPageProps> = ({ onClose, user }) => {
  const [activeTab, setActiveTab] = useState<'pending' | 'successful'>('pending');

  const records = user?.withdrawals || [];
    ...
    return (
      <div className="p-4 space-y-3">
        {filteredRecords.map((record, index) => (
          <div key={index} className="bg-white p-4 rounded-lg shadow-sm animate-fade-in-up">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">To: {record.wallet.name} ({record.wallet.accountNumber})</p>
              </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <header className="bg-white flex items-center justify-between p-4 border-b">
        ...
        <h1 className="text-lg font-semibold text-gray-800">Withdrawal Records</h1>
